#include "head_soft_controller/head_soft_controller.h"
#include <sstream>
#include <pluginlib/class_list_macros.h>

// Register controller to pluginlib
PLUGINLIB_DECLARE_CLASS(head_soft_controller,HeadSoftController, controller::HeadSoftController, pr2_controller_interface::Controller)

namespace controller {

HeadSoftController::HeadSoftController()
...
void HeadSoftController::commandCB(const pr2_soft_controller::SM_TRAJ_STR_ROSConstPtr& msg)
{
...
void HeadSoftController::timeScaleCB(const std_msgs::Float64ConstPtr& msg)
{
  timeScale_ = msg->data;
}

void HeadSoftController::inhibitionCB(const std_msgs::Float64ConstPtr& inhibitionPoints)
{
  --inhibitionPoints_;
}

// Controller initialization in non-realtime
bool HeadSoftController::init(pr2_mechanism_model::RobotState *robot,
                            ros::NodeHandle &n)
{
...
/********************************* Connexion to trajectory and timescale topics ***************/
  ...
  commandSub_= node_.subscribe<pr2_soft_controller::SM_TRAJ_STR_ROS>("command", 1, &HeadSoftController::commandCB, this);
  timeScaleSub_ = node_.subscribe<std_msgs::Float64>("timescale", 1, &HeadSoftController::timeScaleCB, this);
  inhibitionSub_ = node_.subscribe<std_msgs::Float64>("/controllers_inhibition", 1, &HeadSoftController::inhibitionCB, this); 
...
void HeadSoftController::starting()
{
  boost::shared_ptr<SM_TRAJ> newMotionPtr(new SM_TRAJ);
  SM_TRAJ &newMotion = *newMotionPtr;
  ...
  tmpTraj.trajId= -1; 
  tmpTraj.nbAxis= joints_.size();
  tmpTraj.timePreserved= 0;
  for(size_t i=0; i<joints_.size(); ++i){
    tmpTraj.qStart[i] = joints_[i]->position_;
    tmpTraj.qGoal[i] = joints_[i]->position_;
    tmpTraj.traj[i].nbSeg= 1;
    tmpTraj.traj[i].unsused= 0;
    for(size_t j=0; j<1; ++j){
      tmpTraj.traj[i].seg[0].lpId= 0;
      tmpTraj.traj[i].seg[0].unused= 0;
      tmpTraj.traj[i].seg[0].timeOnTraj= 0;
      tmpTraj.traj[i].seg[0].time= 0;
      tmpTraj.traj[i].seg[0].ic_a= 0;
      tmpTraj.traj[i].seg[0].ic_v= 0;
      tmpTraj.traj[i].seg[0].ic_x= joints_[i]->position_;
      tmpTraj.traj[i].seg[0].jerk= 0;
    }
  }
...
void HeadSoftController::update()
{
